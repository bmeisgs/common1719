...
class bankAccount {
     ...
     * @returns {bankAccount}
     * 
     * the method declaration begins with the static keyword, so this method:
     * - won't have access to *this*, as it will not be tied to any instance
     * - can be called by calling bankAccount.createAccount(...), as the identifier it's connected to is bankAccount, not an instance
     */
	 ...
	 * When we call new className(...), the className.constructor instance method is automatically run, which lets us initialize the object.
    ...
    /**
     * Return an array of all bankAccount objects stored in bankAccount.accounts.
     * @static
     * 
     * Since that static property is a named array, iterating through it is more complicated than iterating through a simple array.
     * So this static method goes through the keys of the named array, uses only the keys that look like an account number,
     * because they will have bankAccount objects are their corresponding values; as opposed to owner name-based keys, which would
     * have arrays of bankAccount objects as their corresponding values.
     * 
     * @returns {bankAccount[]}
     */
	...
	for(let i=0;i<names.length;i++) {
	     ...
	     * - names[i] will be then a key from bankAccount.accounts (cf the first line of this static method), that is certain to be an account number (cf the names.filter part).
